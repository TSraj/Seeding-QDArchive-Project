import time
import requests
from .config import DATAVERSE_API_BASE, RATE_LIMIT_DELAY, MAX_PAGES, RESULTS_PER_PAGE, TARGET_EXTENSIONS, RAW_DIR, SMART_QUERIES, NON_QUALITATIVE_KEYWORDS
...
def search_dataverse(extension: str, max_pages: int = MAX_PAGES):
    """Search Dataverse records for an extension query."""
    print(f"\n--- Searching Dataverse for query: '{extension}' ---")
    records_found = []
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            data = api_get(f"{DATAVERSE_API_BASE}/search", params)
            ...
            print(f"Error fetching page {page} from Dataverse for '{extension}': {e}")
            break
            
    return records_found

def scrape(extensions=TARGET_EXTENSIONS, max_pages=MAX_PAGES, dry_run=False, max_runtime_hours=None):
    """Orchestrates the scraping, filtering, and downloading process for Dataverse."""
    ...
    for ext in extensions:
        ...
        if max_runtime_hours is not None:
            ...
            if elapsed_hours >= max_runtime_hours:
                ...
                print(f"\n[INFO] Maximum runtime of {runtime_display} reached. Stopping Dataverse scraper.")
                break
                
        files = search_dataverse(ext, max_pages)
        print(f"Found {len(files)} files for query '{ext}' in Dataverse. Processing...")
        ...
        for file_item in files:
            ...
            dataset_title = file_item.get("dataset_citation", f"Dataverse_Dataset_{dataset_id}").split(',')[0].strip() # Take the first part of the citation (usually Author / Title)
            ...
            record_id = f"dv_{doi}" # Use DOI as the unique identifier for the processed dataset
            ...
            if (RAW_DIR / folder_name_check).exists():
                print(f"Skipping Dataverse Dataset '{dataset_title}': Folder '{folder_name_check}' already exists in raw/. Skipping.")
                continue

            # Convert record_id to an integer offset (e.g. 1 billion base to avoid clashes)
            # Use hash of DOI to ensure determinism
            db_record_id = (hash(doi) % (10**8)) + 1000000000
                
            if not dry_run and is_downloaded(db_record_id):
                print(f"Skipping Dataverse Dataset {db_record_id}: Already completely downloaded and logged in DB.")
                continue
            ...
            try:
                dataset_url = f"{DATAVERSE_API_BASE}/datasets/:persistentId/?persistentId={doi}"
            ...
            for f in all_files:
                ...
                files_to_download.append({
                    ...
                    "links": {
                        "content": f"{DATAVERSE_API_BASE}/access/datafile/{datafile.get('id')}"
                    }
                })
                        
            # FALSE POSITIVE CHECK: The Search API returned this dataset, but the dataset itself doesn't contain the target extension.
            # This happens because Dataverse search matches on metadata text (like description) instead of strict file extensions.
            ...
            if total_dl > 0:
                ...
                project_url = f"{DATAVERSE_API_BASE.replace('/api', '')}/dataset.xhtml?persistentId={doi}"
                
                insert_project(
                    project_id=db_record_id,
                    query_string=ext,
                    repository_name="Dataverse",
                    repository_url=DATAVERSE_API_BASE.replace('/api', ''),
...
def search_smart(query: str, max_pages: int = MAX_PAGES):
    """Search Dataverse using a smart query, returning both file and dataset results."""
    print(f"\n--- Smart Query on Dataverse: '{query}' ---")
    records_found = []
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            ...
            url = f"{DATAVERSE_API_BASE}/search"
            ...
            print(f"Error fetching page {page} from Dataverse for smart query '{query}': {e}")
            break
...
def scrape_smart(queries=SMART_QUERIES, max_pages=MAX_PAGES, dry_run=False, max_runtime_hours=None):
    """Orchestrates smart-query-based scraping for Dataverse.
    ...
    for query in queries:
        ...
        if max_runtime_hours is not None:
            ...
            if elapsed_hours >= max_runtime_hours:
                runtime_display = f"{max_runtime_hours * 60:.0f} minutes" if max_runtime_hours < 1 else f"{max_runtime_hours:.2f} hours"
                print(f"\n[INFO] Maximum runtime of {runtime_display} reached. Stopping Dataverse smart query scraper.")
                break
                
        results = search_smart(query, max_pages)
        print(f"Found {len(results)} results for smart query '{query}' in Dataverse. Processing...")
        
        for item in results:
            # Extract DOI — works for both file and dataset result types
            if item.get("type") == "dataset":
                doi = item.get("global_id", "")
                dataset_title = item.get("name", f"Dataverse_Dataset_{doi}")
            else:
                # File result
                doi = item.get("dataset_persistent_id", "")
                dataset_title = item.get("dataset_citation", f"Dataverse_Dataset_{doi}").split(',')[0].strip()
            
            if not doi:
                continue
                
            record_id = f"dv_smart_{doi}"
            ...
            if (RAW_DIR / folder_name_check).exists():
                print(f"Skipping Dataverse Dataset '{dataset_title}': Folder already exists in raw/. Skipping.")
                continue
            
            db_record_id = (hash(doi) % (10**8)) + 2000000000  # Different base to avoid clashes with extension scraper
                
            if not dry_run and is_downloaded(db_record_id):
                print(f"Skipping Dataverse Smart Query Dataset {db_record_id}: Already downloaded.")
                continue
