import time
import requests
from .config import AUSSDA_API_BASE, RATE_LIMIT_DELAY, MAX_PAGES, RESULTS_PER_PAGE, TARGET_EXTENSIONS, RAW_DIR, SMART_QUERIES, NON_QUALITATIVE_KEYWORDS
...
def search_aussda(extension: str, max_pages: int = MAX_PAGES):
    """Search AUSSDA records for an extension query."""
    print(f"\n--- Searching AUSSDA for query: '{extension}' ---")
    records_found = []
    
    for page in range(1, max_pages + 1):
        # AUSSDA uses start/per_page for pagination
        ...
        try:
            data = api_get(f"{AUSSDA_API_BASE}/search", params)
            ...
            print(f"Error fetching page {page} from AUSSDA for '{extension}': {e}")
            break
            
    return records_found

def scrape(extensions=TARGET_EXTENSIONS, max_pages=MAX_PAGES, dry_run=False, max_runtime_hours=None):
    """Orchestrates the scraping, filtering, and downloading process for AUSSDA."""
    ...
    for ext in extensions:
        ...
        if max_runtime_hours is not None:
            ...
            if elapsed_hours >= max_runtime_hours:
                ...
                print(f"\n[INFO] Maximum runtime of {runtime_display} reached. Stopping AUSSDA scraper.")
                break
                
        files = search_aussda(ext, max_pages)
        print(f"Found {len(files)} files for query '{ext}' in AUSSDA. Processing...")
        
        for file_item in files:
            # AUSSDA search returns files. We group them by dataset using the DOI.
            dataset_id = file_item.get("dataset_citation", "Unknown_Dataset")
            
            dataset_title = file_item.get("dataset_citation", f"AUSSDA_Dataset_{dataset_id}").split(',')[0].strip() # Take the first part of the citation (usually Author / Title)
            ...
            record_id = f"aus_{doi}" # Use DOI as the unique identifier for the processed dataset
            ...
            if (RAW_DIR / folder_name_check).exists():
                print(f"Skipping AUSSDA Dataset '{dataset_title}': Folder '{folder_name_check}' already exists in raw/. Skipping.")
                continue

            # Convert record_id to an integer offset (e.g. 1 billion base to avoid clashes)
            # Use hash of DOI to ensure determinism
            db_record_id = (hash(doi) % (10**8)) + 4000000000
                
            if not dry_run and is_downloaded(db_record_id):
                print(f"Skipping AUSSDA Dataset {db_record_id}: Already completely downloaded and logged in DB.")
                continue
            ...
            try:
                dataset_url = f"{AUSSDA_API_BASE}/datasets/:persistentId/?persistentId={doi}"
            ...
            for f in all_files:
                ...
                files_to_download.append({
                    ...
                    "links": {
                        "content": f"{AUSSDA_API_BASE}/access/datafile/{datafile.get('id')}"
                    }
                })
                        
            # FALSE POSITIVE CHECK: The Search API returned this dataset, but the dataset itself doesn't contain the target extension.
            # This happens because AUSSDA search matches on metadata text (like description) instead of strict file extensions.
            ...
            if total_dl > 0:
                ...
                project_url = f"{AUSSDA_API_BASE.replace('/api', '')}/dataset.xhtml?persistentId={doi}"
                
                insert_project(
                    project_id=db_record_id,
                    query_string=ext,
                    repository_name="AUSSDA",
                    repository_url=AUSSDA_API_BASE.replace('/api', ''),
...
def search_smart(query: str, max_pages: int = MAX_PAGES):
    """Search AUSSDA using a smart query, returning both file and dataset results."""
    print(f"\n--- Smart Query on AUSSDA: '{query}' ---")
    records_found = []
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            ...
            print(f"Error fetching page {page} from AUSSDA for smart query '{query}': {e}")
            break
...
def scrape_smart(queries=SMART_QUERIES, max_pages=MAX_PAGES, dry_run=False, max_runtime_hours=None):
    """Orchestrates smart-query-based scraping for AUSSDA."""
    processed_dataset_ids = set()
    start_time = time.time()
    ...
    for query in queries:
        if max_runtime_hours is not None:
            elapsed_hours = (time.time() - start_time) / 3600
            if elapsed_hours >= max_runtime_hours:
                print("\n[INFO] Maximum runtime reached. Stopping AUSSDA smart query scraper.")
                break
                
        results = search_smart(query, max_pages)
        print(f"Found {len(results)} results for smart query '{query}' in AUSSDA. Processing...")
        
        for item in results:
            if item.get("type") == "dataset":
                doi = item.get("global_id", "")
                dataset_title = item.get("name", f"AUSSDA_Dataset_{doi}")
            else:
                doi = item.get("dataset_persistent_id", "")
                dataset_title = item.get("dataset_citation", f"AUSSDA_Dataset_{doi}").split(',')[0].strip()
            
            if not doi:
                continue
                
            record_id = f"aus_smart_{doi}"
            ...
            if (RAW_DIR / folder_name_check).exists():
                print(f"Skipping AUSSDA Dataset '{dataset_title}': Folder already exists.")
                continue
            
            db_record_id = (hash(doi) % (10**8)) + 4500000000
                
            if not dry_run and is_downloaded(db_record_id):
                print(f"Skipping AUSSDA Smart Query Dataset {db_record_id}: Already downloaded.")
                continue
