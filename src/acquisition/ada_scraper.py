import time
import requests
from .config import ADA_API_BASE, ADA_API_TOKEN, RATE_LIMIT_DELAY, MAX_PAGES, RESULTS_PER_PAGE, TARGET_EXTENSIONS, RAW_DIR, SMART_QUERIES, NON_QUALITATIVE_KEYWORDS
...
def _build_auth_headers():
    """Build authentication headers for ADA Dataverse API."""
    headers = {}
    if ADA_API_TOKEN:
        headers["X-Dataverse-key"] = ADA_API_TOKEN
    return headers
...
def search_ada(extension: str, max_pages: int = MAX_PAGES):
    """Search ADA records for an extension query."""
    print(f"\n--- Searching ADA for query: '{extension}' ---")
    records_found = []
    
    for page in range(1, max_pages + 1):
        # ADA uses start/per_page for pagination
        ...
        try:
            data = api_get(f"{ADA_API_BASE}/search", params, headers=_build_auth_headers())
            ...
            print(f"Error fetching page {page} from ADA for '{extension}': {e}")
            break
            
    return records_found

def scrape(extensions=TARGET_EXTENSIONS, max_pages=MAX_PAGES, dry_run=False, max_runtime_hours=None):
    """Orchestrates the scraping, filtering, and downloading process for ADA."""
    ...
    for ext in extensions:
        ...
        if max_runtime_hours is not None:
            ...
            if elapsed_hours >= max_runtime_hours:
                ...
                print(f"\n[INFO] Maximum runtime of {runtime_display} reached. Stopping ADA scraper.")
                break
                
        files = search_ada(ext, max_pages)
        print(f"Found {len(files)} files for query '{ext}' in ADA. Processing...")
        
        for file_item in files:
            # ADA search returns files. We group them by dataset using the DOI.
            dataset_id = file_item.get("dataset_citation", "Unknown_Dataset")
            
            dataset_title = file_item.get("dataset_citation", f"ADA_Dataset_{dataset_id}").split(',')[0].strip() # Take the first part of the citation (usually Author / Title)
            ...
            record_id = f"ada_{doi}" # Use DOI as the unique identifier for the processed dataset
            ...
            if (RAW_DIR / folder_name_check).exists():
                print(f"Skipping ADA Dataset '{dataset_title}': Folder '{folder_name_check}' already exists in raw/. Skipping.")
                continue

            # Convert record_id to an integer offset (e.g. 1 billion base to avoid clashes)
            # Use hash of DOI to ensure determinism
            db_record_id = (hash(doi) % (10**8)) + 8000000000
                
            if not dry_run and is_downloaded(db_record_id):
                print(f"Skipping ADA Dataset {db_record_id}: Already completely downloaded and logged in DB.")
                continue
            ...
            try:
                dataset_url = f"{ADA_API_BASE}/datasets/:persistentId/?persistentId={doi}"
                dataset_data = api_get(dataset_url, headers=_build_auth_headers())
            ...
            for f in all_files:
                ...
                files_to_download.append({
                    ...
                    "links": {
                        "content": f"{ADA_API_BASE}/access/datafile/{datafile.get('id')}"
                    }
                })
                        
            # FALSE POSITIVE CHECK: The Search API returned this dataset, but the dataset itself doesn't contain the target extension.
            # This happens because ADA search matches on metadata text (like description) instead of strict file extensions.
            ...
            # Perform Download
            total_dl, total_bytes, folder_name, downloaded_files = download_record(dataset_record, files_to_download, headers=_build_auth_headers())
            
            if total_dl > 0:
                parsed_meta = extract_dv_meta(dataset_record)
                project_url = f"{ADA_API_BASE.replace('/api', '')}/dataset.xhtml?persistentId={doi}"
                
                insert_project(
                    project_id=db_record_id,
                    query_string=ext,
                    repository_name="ADA",
                    repository_url=ADA_API_BASE.replace('/api', ''),
...
def is_qualitative_dataset(dataset_data):
    ...
    Returns False if the dataset title or description contains non-qualitative keywords."""
    latest = dataset_data.get("latestVersion", {})
    blocks = latest.get("metadataBlocks", {})
    citation_fields = blocks.get("citation", {}).get("fields", [])
    
    # Gather text to check: title + description
    title = ""
    description = ""
...
def search_smart(query: str, max_pages: int = MAX_PAGES):
    """Search ADA using a smart query, returning both file and dataset results."""
    print(f"\n--- Smart Query on ADA: '{query}' ---")
    records_found = []
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            # Search both files and datasets by passing type twice
            url = f"{ADA_API_BASE}/search"
            ...
            time.sleep(RATE_LIMIT_DELAY)
            response = requests.get(full_url, headers=_build_auth_headers(), timeout=30)
            response.raise_for_status()
            data = response.json()
            ...
            print(f"Error fetching page {page} from ADA for smart query '{query}': {e}")
            break
...
def scrape_smart(queries=SMART_QUERIES, max_pages=MAX_PAGES, dry_run=False, max_runtime_hours=None):
    """Orchestrates smart-query-based scraping for ADA.
    Searches using keyword/boolean queries to find qualitative datasets
    that may not contain files with known QDA extensions."""
    processed_dataset_ids = set()
    start_time = time.time()
    
    for query in queries:
        # Check runtime before starting a new query
        if max_runtime_hours is not None:
            elapsed_hours = (time.time() - start_time) / 3600
            if elapsed_hours >= max_runtime_hours:
                runtime_display = f"{max_runtime_hours * 60:.0f} minutes" if max_runtime_hours < 1 else f"{max_runtime_hours:.2f} hours"
                print(f"\n[INFO] Maximum runtime of {runtime_display} reached. Stopping ADA smart query scraper.")
                break
                
        results = search_smart(query, max_pages)
        print(f"Found {len(results)} results for smart query '{query}' in ADA. Processing...")
        
        for item in results:
            # Extract DOI — works for both file and dataset result types
            if item.get("type") == "dataset":
                doi = item.get("global_id", "")
                dataset_title = item.get("name", f"ADA_Dataset_{doi}")
            else:
                # File result
                doi = item.get("dataset_persistent_id", "")
                dataset_title = item.get("dataset_citation", f"ADA_Dataset_{doi}").split(',')[0].strip()
            
            if not doi:
                continue
                
            record_id = f"ada_smart_{doi}"
            ...
            if (RAW_DIR / folder_name_check).exists():
                print(f"Skipping ADA Dataset '{dataset_title}': Folder already exists in raw/. Skipping.")
                continue
            
            db_record_id = (hash(doi) % (10**8)) + 9000000000  # Different base to avoid clashes with extension scraper
                
            if not dry_run and is_downloaded(db_record_id):
                print(f"Skipping ADA Smart Query Dataset {db_record_id}: Already downloaded.")
                continue
            ...
            # Construct file list for download (no extension filtering — download everything)
            files_to_download = []
            ...
            # Prepare dataset record metadata
            dataset_record = dataset_data.get("data", {})
