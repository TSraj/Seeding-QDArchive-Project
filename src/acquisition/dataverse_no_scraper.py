import time
import requests
from .config import DATAVERSE_NO_API_BASE, RATE_LIMIT_DELAY, MAX_PAGES, RESULTS_PER_PAGE, TARGET_EXTENSIONS, RAW_DIR, SMART_QUERIES, NON_QUALITATIVE_KEYWORDS
    ...
    for ext in extensions:
        ...
        for file_item in files:
            ...
            if total_dl > 0:
                ...
                project_url = f"{DATAVERSE_NO_API_BASE.replace('/api', '')}/dataset.xhtml?persistentId={doi}"
                
                insert_project(
                    project_id=db_record_id,
                    query_string=ext,
                    repository_name="DataverseNO",
                    repository_url=DATAVERSE_NO_API_BASE.replace('/api', ''),
...
def search_smart(query: str, max_pages: int = MAX_PAGES):
    """Search Dataverse NO using a smart query, returning both file and dataset results."""
    print(f"\n--- Smart Query on Dataverse NO: '{query}' ---")
    records_found = []
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            url = f"{DATAVERSE_NO_API_BASE}/search"
            ...
            print(f"Error fetching page {page} from Dataverse NO for smart query '{query}': {e}")
            break
...
def scrape_smart(queries=SMART_QUERIES, max_pages=MAX_PAGES, dry_run=False, max_runtime_hours=None):
    """Orchestrates smart-query-based scraping for Dataverse NO."""
    processed_dataset_ids = set()
    start_time = time.time()
    ...
    for query in queries:
        if max_runtime_hours is not None:
            elapsed_hours = (time.time() - start_time) / 3600
            if elapsed_hours >= max_runtime_hours:
                print("\n[INFO] Maximum runtime reached. Stopping Dataverse NO smart query scraper.")
                break
                
        results = search_smart(query, max_pages)
        print(f"Found {len(results)} results for smart query '{query}' in Dataverse NO. Processing...")
        
        for item in results:
            if item.get("type") == "dataset":
                doi = item.get("global_id", "")
                dataset_title = item.get("name", f"Dataverse_NO_Dataset_{doi}")
            else:
                doi = item.get("dataset_persistent_id", "")
                dataset_title = item.get("dataset_citation", f"Dataverse_NO_Dataset_{doi}").split(',')[0].strip()
            
            if not doi:
                continue
                
            record_id = f"dno_smart_{doi}"
            ...
            if (RAW_DIR / folder_name_check).exists():
                print(f"Skipping Dataverse NO Dataset '{dataset_title}': Folder already exists.")
                continue
            
            # Base logic offset for DNO is 2 billion from the extension matching script + 1 billion to avoid clash
            # Using 2500000000 for DNO Smart
            db_record_id = (hash(doi) % (10**8)) + 2500000000
                
            if not dry_run and is_downloaded(db_record_id):
                print(f"Skipping Dataverse NO Smart Query Dataset {db_record_id}: Already downloaded.")
                continue
